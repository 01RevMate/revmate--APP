...
export async function fetchFeed(limit = 20): Promise<PostWithAuthor[]> {
  const { data, error } = await supabase
    .from("posts")
    .select("*, profiles!posts_user_id_fkey(username, avatar_url), cars(make, model, generation)")
...
export async function createPost(input: { userId: string; body: string; carId?: string | undefined }) {
