...
export function GarageCarTile({ username, car }: { username: string; car: GarageCar }) {
  ...
  return (
    <Link
      ...
      className={`group relative flex flex-col items-center gap-2 overflow-hidden rounded-lg border border-border bg-card pb-4 pt-6 text-center transition-colors hover:bg-accent ${isPrevious ? "opacity-75" : ""}`}
    >
